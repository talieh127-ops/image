...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!process.env.API_KEY) {
      setStatus(prev => ({ ...prev, error: "API_KEY یافت نشد. لطفاً در تنظیمات محیطی (Environment Variables) آن را تعریف کنید." }));
      return;
    }
    ...
    try {
      ...
      setStatus({
        isGenerating: false,
        error: err.message || "خطایی رخ داد. احتمالاً کلید API نامعتبر است یا سهمیه شما تمام شده است.",
        currentMessage: ""
      });
    }
  };
  ...
  return (
    <div className="min-h-screen pb-20 px-4 sm:px-6 lg:px-8">
      ...
      {!isApiKeySet && (
        <div className="max-w-4xl mx-auto mt-4 p-4 bg-amber-500/10 border border-amber-500/50 rounded-xl text-amber-200 text-sm flex items-center justify-between" dir="rtl">
          <div className="flex items-center gap-3">
            <i className="fas fa-exclamation-triangle text-amber-500 text-xl"></i>
            <p><strong>هشدار مهم:</strong> متغیر <code>API_KEY</code> شناسایی نشد. این برنامه در حالت انتشار کار نخواهد کرد مگر اینکه کلید را در تنظیمات پلتفرم خود وارد کنید.</p>
          </div>
          <button onClick={() => window.scrollTo({top: document.body.scrollHeight, behavior: 'smooth'})} className="bg-amber-500 text-slate-900 px-3 py-1 rounded-md font-bold text-xs">چطور تنظیم کنم؟</button>
        </div>
      )}
      ...
      <header className="max-w-7xl mx-auto pt-8 pb-12 flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-4 bg-slate-900 px-3 py-1 rounded-full border border-slate-800 text-[10px] uppercase tracking-widest text-slate-500 font-bold">
          <span className={`w-2 h-2 rounded-full ${isApiKeySet ? 'bg-green-500' : 'bg-red-500'}`}></span>
          {isApiKeySet ? 'API Connection Active' : 'API Connection Missing'}
        </div>
        ...
        <section className="max-w-3xl mx-auto glass p-6 rounded-3xl space-y-6 shadow-2xl shadow-indigo-500/5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              disabled={status.isGenerating || !prompt.trim() || !isApiKeySet}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {history.map((item) => (
                <div key={item.id} className="group relative glass rounded-2xl overflow-hidden flex flex-col h-full hover:ring-2 hover:ring-indigo-500/50 transition duration-300">
                  <div className={`relative overflow-hidden aspect-[${item.aspectRatio === '1:1' ? '1/1' : item.aspectRatio.replace(':', '/')}] bg-slate-900`}>
                    <img
                      src={item.url}
                      alt={item.prompt}
